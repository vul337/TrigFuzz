...
u8 *describe_op(afl_state_t *afl, u8 new_bits, size_t max_description_len) {
  ...
  if (afl->trigfuzz_save_plus_trig) { strcat(ret, ",+trig"); }
...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    trig_interesting = trigfuzz_current_is_interesting(afl);

    if (likely(!new_bits)) {

      if (san_fault == FSRV_RUN_OK && !trig_interesting) {

        if (unlikely(afl->crash_mode)) { ++afl->total_crashes; }
        return 0;

      } else if (san_fault != FSRV_RUN_OK) {
  ...
  save_to_queue:
    ...
    afl->trigfuzz_save_plus_trig = trig_interesting;
    ...
    trigfuzz_update_queue_entry(afl, afl->queue_top, trig_interesting);
    afl->trigfuzz_save_plus_trig = 0;
