...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  trigfuzz_note_queue_filename(afl, q);
...
inline void cull_queue(afl_state_t *afl) {
  ...
  for (i = 0; i < afl->queued_items; i++) {
    ...
    q->favored = q->tightness_novel || trigfuzz_queue_is_favored(afl, q);
    if (q->favored && !q->disabled) {
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  double trigfuzz_multiplier = trigfuzz_perf_multiplier(afl, q);
  if (trigfuzz_multiplier > 1.0) {

    perf_score = (u32)((double)perf_score * trigfuzz_multiplier);
