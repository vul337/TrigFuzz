...
static std::string canonicalBBName(std::string Name) {
...
static std::string basenamePath(std::string Filename) {
...
static bool isSystemPath(const std::string &Filename) {

  static const std::string Xlibs("/usr/");
  return !Filename.compare(0, Xlibs.size(), Xlibs);

}

static void getInstructionDebugLoc(const Instruction *I, std::string &Filename,
                                   unsigned &Line) {
...
static bool instructionDebugBBName(const Instruction *I, std::string &BBName,
                                   bool SkipSystemPaths) {

  std::string Filename;
  unsigned    Line = 0;
  getInstructionDebugLoc(I, Filename, Line);

  if (Filename.empty() || Line == 0) return false;
  if (SkipSystemPaths && isSystemPath(Filename)) return false;

  BBName = basenamePath(Filename) + ":" + std::to_string(Line);
  BBName = canonicalBBName(BBName);
  return !BBName.empty();

}

static std::string basicBlockDebugName(BasicBlock &BB,
                                       bool SkipSystemPaths) {

  for (auto &I : BB) {

    std::string BBName;
    if (instructionDebugBBName(&I, BBName, SkipSystemPaths)) return BBName;
...
static bool parseTrigFuzzSelectiveSetting(const char *Value,
                                          std::string &Mode) {

  if (!Value) return false;

  std::string Setting = lowerString(trimString(Value));
  ...
  if (Setting == "conservative" || Setting == "selectfuzz-like" ||
      Setting == "selectfuzz") {
  ...
  FATAL(
      "Bad value of AFL_TRIGFUZZ_SELECTIVE (use 0, 1, conservative, or "
      "selectfuzz-like)");
  return false;

}

static bool loadSelectiveKeepList(const char *Path,
                                  std::set<std::string> &KeepList) {
  ...
  while (std::getline(Input, Line)) {
    ...
    std::string BBName = canonicalBBName(Line);
    if (!BBName.empty()) KeepList.insert(BBName);
...
PreservedAnalyses AFLCoverage::run(Module &M, ModuleAnalysisManager &MAM) {
  ...
  */
  ...
  std::string           selective_mode;
  std::set<std::string> selective_blocks;
  bool                  is_selective = parseTrigFuzzSelectiveSetting(
      getenv("AFL_TRIGFUZZ_SELECTIVE"), selective_mode);
  if (is_selective) {

    const char *selective_file = getenv("AFL_TRIGFUZZ_SELECTIVE_FILE");
    if (!selective_file) selective_file = getenv("AFLGO_SELECTIVE_FILE");
    if (!loadSelectiveKeepList(selective_file, selective_blocks))
      FATAL(
          "AFL_TRIGFUZZ_SELECTIVE requires a non-empty "
  ...
  for (auto &F : M) {
    ...
    for (auto &BB : F) {
      ...
      if (is_selective) {
        ...
        std::string BBName = basicBlockDebugName(BB, true);
        if (!BBName.empty() && !selective_blocks.count(BBName)) {
  ...
  */
  ...
  if (!be_quiet) {
    ...
    if (!inst_blocks)
      ...
      if (is_selective)
        OKF(
            "Instrumented %d locations (%s mode, ratio %u%%, TrigFuzz "
            "selective=%s, skipped=%d).",
            inst_blocks, modeline, inst_ratio, selective_mode.c_str(),
            selective_skipped);
      else
