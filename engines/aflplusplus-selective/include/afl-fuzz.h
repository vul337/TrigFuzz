...
struct queue_entry {
  ...
  bool trim_done,                       /* Trimmed?                         */
      ...
      plus_trig,                        /* TrigFuzz distance-improving case */
      trig_distance_valid,              /* TrigFuzz distance was measured   */
  ...
  double perf_score,                    /* performance score                */
      weight,
      trig_distance;                    /* TrigFuzz triggering distance     */
  ...
  u32 trig_novel_cycle;                 /* cycle when plus_trig was set     */
...
typedef struct afl_state {
  ...
  u8 havoc_stack_pow2,                  /* HAVOC_STACK_POW2                 */
      ...
      trigfuzz_enabled,                 /* TrigFuzz feedback enabled        */
      trigfuzz_debug,                   /* verbose TrigFuzz diagnostics     */
      trigfuzz_cur_distance_valid,      /* latest run has TrigFuzz distance */
      trigfuzz_cur_plus,                /* latest run improved distance     */
      trigfuzz_save_plus_trig,          /* describe current save as +trig   */
      trigfuzz_best_distance_valid,     /* best distance initialized        */
      trigfuzz_distance_range_valid;    /* min/max distance initialized     */

  u8 *virgin_bits,                      /* Regions yet untouched by fuzzing */
      *virgin_tmout,                    /* Bits we haven't seen in tmouts   */
      *virgin_crash,                    /* Bits we haven't seen in crashes  */
      *trigfuzz_map;                    /* TrigFuzz distance shm            */

  double *alias_probability;            /* alias weighted probabilities     */
  double trigfuzz_cur_distance,         /* latest execution triggering dist */
      trigfuzz_best_distance,           /* best saved/calibrated trig dist  */
      trigfuzz_min_distance,            /* min measured queue trig dist     */
      trigfuzz_max_distance;            /* max measured queue trig dist     */
  u32    *alias_table;                /* alias weighted random lookup table */
  u32     alias_map_size;             /* allocated capacity of alias arrays */
  size_t  trigfuzz_map_size;          /* allocated TrigFuzz shm size       */
  u32     trigfuzz_ins_num,           /* number of TCU slots               */
      trigfuzz_seq_num,               /* number of execution-order slots   */
      trigfuzz_saved_plus_trig;       /* saved distance-improving inputs   */
...
void   trigfuzz_init(afl_state_t *);
void   trigfuzz_deinit(afl_state_t *);
void   trigfuzz_reset(afl_state_t *);
void   trigfuzz_after_run(afl_state_t *);
u8     trigfuzz_current_is_interesting(afl_state_t *);
void   trigfuzz_update_queue_entry(afl_state_t *, struct queue_entry *, u8);
void   trigfuzz_note_queue_filename(afl_state_t *, struct queue_entry *);
u8     trigfuzz_queue_is_favored(afl_state_t *, struct queue_entry *);
double trigfuzz_perf_multiplier(afl_state_t *, struct queue_entry *);
